...
def setup(parameter_set,
    ...
    AS_motors = []
    TW_cells = ['AVAL', 'AVAR', 'AVBL', 'AVBR', 'AVDL', 'AVDR', 'DVA', 'PVDL', 'PVDR', 'PLML', 'PLMR',
                'AVM', 'ALML', 'ALMR']  # PATCHED: PVCL, PVCR removed (ablation)
    ...
    reference = "c302_%s_TouchTW_PVCablated_AVBstim" % parameter_set
    ...
    if generate:
        ...
        # PATCHED (bypass control): inject directly into AVB instead of PLM, to test
        # whether the motor layer can still respond when the sensory->PVC->AVB relay
        # is skipped entirely. If PVC-ablated + AVB-direct-stim responds normally while
        # PVC-ablated + PLM-stim does not, the break is localised precisely to PVC.
        c302.add_new_input(nml_doc, "AVBL", "50ms", "300ms", stim_amplitude, params)
        c302.add_new_input(nml_doc, "AVBR", "50ms", "300ms", stim_amplitude, params)
