...
def setup(parameter_set,
    ...
    # PATCHED for touch-circuit ablation study: enable posterior touch stimulus (PLM),
    # which is disabled (cells_to_stimulate=[], all add_new_input calls commented out) upstream.
    ...
    AS_motors = []
    TW_cells = ['AVBL', 'AVBR', 'PVCL', 'PVCR', 'AVDL', 'AVDR', 'DVA', 'PVDL', 'PVDR', 'PLML', 'PLMR',
                'AVM', 'ALML', 'ALMR']  # PATCHED: AVAL, AVAR removed (ablation)
    ...
    muscles_to_include = True  # PATCHED: enable muscles so neural activity drives the body
    ...
    reference = "c302_%s_TouchTW_AVAablated_ALMstim" % parameter_set
    ...
    if generate:
        ...
        #c302.add_new_input(nml_doc, "PLML", "50ms", "300ms", stim_amplitude, params)
        #c302.add_new_input(nml_doc, "PLMR", "50ms", "300ms", stim_amplitude, params)
        # PATCHED (ALM variant): anterior touch stimulus, to test AVA's actual
        # expected role (reversal pathway) instead of the default posterior/PLM protocol.
        c302.add_new_input(nml_doc, "ALML", "50ms", "300ms", stim_amplitude, params)
        c302.add_new_input(nml_doc, "ALMR", "50ms", "300ms", stim_amplitude, params)
        c302.add_new_input(nml_doc, "AVM", "50ms", "300ms", stim_amplitude, params)
